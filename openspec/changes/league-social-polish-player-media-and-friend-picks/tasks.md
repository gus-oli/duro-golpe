...
- [x] 6.1 Reproduce and fix the internal server error when opening friend picks.
- [x] 6.2 Add regression coverage for friend-pick DTO shape, league membership authorization, empty picks, and failed lookups.
- [x] 6.3 Reproduce and fix `/outrights` rendering with no available markets despite configured markets existing.
- [x] 6.4 Ensure migrations needed by friend picks, outright media, and account management are present, idempotent, and documented for Neon/Render rollout.
- [x] 6.5 Add backend/frontend fallbacks so missing optional media fields or invalid placeholder options do not collapse the entire outrights page.

## 7. Authenticated navigation shell cleanup

- [x] 7.1 Replace repeated large navigation cards on internal authenticated pages with a compact hamburger/drawer navigation.
- [x] 7.2 Keep large destination cards only on the authenticated home/Inicio surface.
- [x] 7.3 Ensure the drawer exposes Inicio, Partidas, Ligas, Especiais, and Conta with active-route treatment.
- [x] 7.4 Make the drawer keyboard-accessible, mobile-safe, and auto-closing after navigation.
- [x] 7.5 Remove redundant Partidas, Ligas, Especiais, and Conta cards from matches, league, outrights, and profile pages.

## 8. Product surface hierarchy polish

- [x] 8.1 Remove the Home Atalhos section and expand Proxima acao with every match from the next relevant date.
- [x] 8.2 Add direct action states in Proxima acao for open predictions, locked/live matches, and finished results.
- [x] 8.3 Replace `/matches` large tab cards with a compact segmented control for Agenda, Grupos, and Resultados.
- [x] 8.4 Preserve `/matches` results as agenda-first official score browsing.
- [x] 8.5 Reorder `/leagues/[leagueId]` to Pontuacao, Classificacao, then Mural.
- [x] 8.6 Remove internal navigation card clutter from `/outrights` while keeping markets, search, flags/photos, and friend-pick entry points prominent.

## 9. Account profile management

- [x] 9.1 Add authenticated `GET /me` returning the current user's editable account profile.
- [x] 9.2 Add authenticated `PATCH /me` for display name/apelido and e-mail updates.
- [x] 9.3 Validate duplicate e-mail updates with a clear conflict response.
- [x] 9.4 Add authenticated `POST /me/password` requiring the current password and storing the new password hash.
- [x] 9.5 Build `/profile` forms for editing apelido, e-mail, and password with success/error states.
- [x] 9.6 Add tests for profile update authorization, duplicate e-mail, wrong current password, and successful password change.
...
- [x] 10.2 Run frontend build/lint for navigation and page hierarchy changes.
